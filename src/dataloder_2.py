...
params = { 'batch_size': 16, 'shuffle': True, 'num_workers': 10, 'drop_last': True}
data = {'Data_hz': 2, 'Frame_len': 5}
...
class my_dataset(Dataset):
        ...
        for i in self.csv_list_of_dfs:
            for j in range(len(i)-(self.frame_len-1)*self.data_hz):
                ...
                for k in range(self.frame_len):
                    #tmp_list.append(1)
                    tmp_list.append(i.values[j+k*self.data_hz])
    ...
    def __len__(self):
        #count = 0
        #for i in self.csv_list_of_dfs:
        #    count = count + len(i)-(self.frame_len-1)*self.data_hz
...
if __name__ == "__main__":
    ...
    input_size = 32 * 3 * batch_size * data['Frame_len']
    hidden_size = 512 * batch_size * data['Frame_len']
    num_classes = 10 * batch_size * data['Frame_len']
    num_epochs = 1
    total_step = 6693
    ...
    for epoch in range(max_epochs):
        ...
        with torch.set_grad_enabled(False):
            ...
            PATH = "/media/data/weiling/project/Action_Category_CVG/model/"+str(epoch)+".pth"
